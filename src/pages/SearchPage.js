...
const SearchPage = () => {
    ...
    return (
      <div className="car-link">
        <a href={`/car-info/${car.AdID}`} className="car-info-link">
          <div style={{ height: "70%" }}>
            <img src={car.photoLink[3]} alt="car" />
          </div>
          <div className="car-details-container">
            <div className="car-item-row">
              <p>{car.title}</p>
              <p>{car.tag_block.slice(0, car.tag_block.indexOf(",")).trim()}</p>
            </div>
            <div className="car-item-row">
              <p>
    ...
    return (
      <div className="car-search-info">
        {currentItems.map((item) => (
          <a href={`/car-info/${item.AdID}`} className="car-info-link">
            <div className="car-search-item">
              <img src={item.photoLink?.[2]} alt="car" />
              <div
  ...
  const DisplayedCarsInfo = () => {
    ...
    return (
      <div className="displayed-cars-info">
        <p>
          Prikazano od {start} do {end} oglasa od ukupno {sortedData.length}
        </p>
      </div>
    );
  };
  ...
  const SortSelect = ({ onSort }) => {
    ...
    const handleChange = (label, value) => {
      ...
      console.log("Selected Label is now:", label);
    };
    ...
    console.log("Component rendered. Selected label:", selectedLabel);
